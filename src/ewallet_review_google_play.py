...
def is_english_review(text: str) -> bool:
    ...
    # Reject uncertain text as well, so only confidently English reviews continue.
    try:
...
def extract_google_play_app_id(app_reference: str) -> str:
    ...
    # Normalize both user-friendly URLs and package IDs into one scraper input.
...
def collect_google_play_reviews(app_id: str, count: int, language: str, country: str) -> pd.DataFrame:
    ...
    # Fetch raw review metadata first, then remove empty, duplicate, and non-English rows.
...
def triage_google_play_reviews(model, reviews_dataframe: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, int]]:
    """Classify 1–3 star reviews while leaving positive reviews untouched."""
    # Use ratings as the sentiment gate before sending complaints to the ML model.
    results = reviews_dataframe.copy()
    results["Sentiment"] = results["Rating"].map(
        lambda rating: "Needs Review" if rating <= 3 else "Positive"
    )
    results["Action"] = results["Sentiment"].map(
        {"Needs Review": "Classified for follow-up", "Positive": "Left unchanged"}
    )
    results["Predicted Issue"] = ""
    results["Confidence"] = None
    negative_rows = results["Sentiment"] == "Needs Review"
    ...
    summary = {
        ...
        "needs_review": int(negative_rows.sum()),
